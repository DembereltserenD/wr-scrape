...
const ChampionCard: React.FC<ChampionCardProps> = ({
    ...
    const getTierColor = (tier: string): string => {
        ...
        return tierColors[tier] || 'bg-gradient-to-r from-yellow-400 to-yellow-500 text-black';
    };
    ...
    return (
        <div className="champion-card gaming-glow group transition-all duration-300 hover:scale-105">
            <div className="relative">
                ...
                <Image
                    src={imageError ? '/placeholder-champion.png' : (champion.image || '/placeholder-champion.png')}
                ...
                <div className="absolute top-2 right-2">
                    <span className={`px-3 py-1 rounded-full text-sm font-bold shadow-lg ${getTierColor(champion.tier)}`}>
                ...
                <div className="absolute top-2 left-2">
                    <span className={`bg-black bg-opacity-60 text-white px-2 py-1 rounded text-xs font-medium ${getDifficultyColor(champion.difficulty)}`}>
            ...
            <div className="p-4">
                ...
                <div className="mb-3">
                    <div className="flex flex-wrap gap-1">
                        {champion.lanes.map((lane, index) => {
                            ...
                            const displayLane = lane === 'Baron' ? 'Baron Lane' :
                                lane === 'Mid' ? 'Mid Lane' :
                                    lane === 'Dragon' ? 'Dragon Lane' :
                                        lane;
                ...
                {(champion.winRate || champion.pickRate) && (
                    <div className="flex justify-between text-sm text-gray-600 dark:text-gray-400 mb-3">
                        {champion.winRate && (
                            <span>{t('champion.win_rate')}: {champion.winRate}</span>
                        )}
                        {champion.pickRate && (
                            <span>{t('champion.pick_rate')}: {champion.pickRate}</span>
                        )}
                    </div>
                )}
