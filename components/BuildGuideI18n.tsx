...
const BuildGuideI18n: React.FC<BuildGuideProps> = ({ builds, runes }) => {
  ...
  const renderItem = (item: any, index: number, showNumber = false) => (
    <div
      key={index}
      className="flex items-center gap-3 bg-black/30 rounded-lg p-3 border border-slate-600/50 hover:border-cyan-500/50 transition-all duration-200"
    >
      {showNumber && (
      ...
      <div className="relative">
        <Tooltip
          content={
            <div className="text-white">
              <h4 className="font-bold text-yellow-300 text-lg mb-2">
                {item.name}
              </h4>
              {item.description && (
              ...
              {item.passive && (
                <div className="text-xs text-purple-300 mt-2">
                  <span className="font-semibold">Passive:</span> {item.passive}
                </div>
              )}
              {item.active && (
        ...
        >
          <img
            ...
            className="w-10 h-10 rounded-lg border-2 border-cyan-500/50 shadow-lg cursor-pointer hover:border-yellow-400/70 transition-all duration-200"
            onError={(e) => {
          ...
          />
          <div className="w-10 h-10 rounded-lg border-2 border-cyan-500/50 bg-gradient-to-br from-slate-600 to-slate-700 hidden items-center justify-center text-white font-bold text-xs cursor-pointer hover:border-yellow-400/70 transition-all duration-200">
            {item.name?.charAt(0)}
          </div>
        </Tooltip>
      </div>
  ...
  const renderRune = (rune: any, index: number, isKeystone = false) => (
    ...
    >
      <div className="relative">
        <Tooltip
          content={
            <div className="text-white">
              <h4
                className={`font-bold text-lg mb-2 ${isKeystone ? "text-yellow-300" : "text-purple-300"}`}
              >
                {rune.name}
                {isKeystone && (
        ...
        >
          <img
            ...
            className="w-8 h-8 rounded-lg border-2 border-cyan-500/50 shadow-lg cursor-pointer hover:border-yellow-400/70 transition-all duration-200"
            onError={(e) => {
              const target = e.target as HTMLImageElement;
              target.style.display = "none";
              const fallback = target.nextElementSibling as HTMLElement;
          ...
          />
          <div className="w-8 h-8 rounded-lg border-2 border-cyan-500/50 bg-gradient-to-br from-slate-600 to-slate-700 hidden items-center justify-center text-white font-bold text-xs cursor-pointer hover:border-yellow-400/70 transition-all duration-200">
            {rune.name?.charAt(0)}
          </div>
        </Tooltip>
      </div>
      <div className="flex-1">
        <h5
          className={`font-semibold text-sm ${isKeystone ? "text-yellow-300" : "text-white"}`}
        >
          {rune.name}
          {isKeystone && (
            <span className="text-xs text-yellow-500 ml-2">
        ...
        <div className="space-y-4">
          ...
          <div>
            <h4 className="font-bold text-pink-300 mb-3 flex items-center gap-2">
              ...
              {t("runes.secondary")} (
              {getRuneTreeTranslation(runes.secondary.tree)})
            </h4>
            <div className="space-y-2">
